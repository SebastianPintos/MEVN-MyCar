              ...
              <v-container>
                ...
                <v-row>
                  ...
                  <v-col cols="12" sm="4" md="3">
                    <v-text-field
                      prefix="$"
                      v-model="filtros.cDesde"
                      label="Precio última Compra Desde"
                    ></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="4" md="3">
                    <v-text-field
                      prefix="$"
                      v-model="filtros.cHasta"
                      label="Precio última Compra Hasta"
                    ></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="4" md="3">
                    <v-text-field
                      prefix="$"
                      v-model="filtros.vDesde"
                  ...
                  <v-col cols="12" sm="4" md="3">
                    <v-text-field
                      prefix="$"
                      v-model="filtros.vHasta"
                      label="Precio de Venta Hasta"
                    ></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="4" md="3">
                    <v-select
                      v-model="filtros.Dealer"
                      :items="dealersList"
                      item-text="Email"
                      item-value="Email"
                      label="Proveedor"
                    ...
                    <v-container>
                      <v-row>
                        ...
                        <v-col cols="12" sm="4" md="6">
                          <v-text-field
                            :rules="reglaPrecio"
                            v-model="editedItem.LastPurchasePrice"
                        ...
                        <v-col cols="12" sm="4" md="12">
                          <v-text-field
                            v-model="editedItem.ShippingDealer"
                            label="Tiempo envío entre Sucursales (días)"
                            :rules="requerido"
                          ></v-text-field>
                        </v-col>
                        ...
                        <v-col cols="12" sm="4" md="12">
                          <v-text-field
                            v-model="editedItem.ShippingBranch"
                            label="Tiempo de envío Proveedor (días)"
                            :rules="requerido"
                          ></v-text-field>
                        </v-col>
                      ...
                      <v-row>
                        ...
                        <v-col cols="12" sm="6" md="10">
                          <v-text-field
                            id="precioVenta"
                            suffix="%"
                            :disabled="deshabilitarPrecioVenta"
                            :rules="reglaEditarVenta"
                            v-model="editedItem.SalePrice"
                            prefix="$"
                            label="Precio de Venta"
                          ></v-text-field>
                        ...
                        <v-col cols="12" sm="6" md="2">
                          <v-btn
                            :class="classBotonVenta"
                            @click="clickPrecioVenta()"
                          >
                        ...
                        <v-col cols="12" sm="6" md="10">
                          <v-text-field
                            :disabled="deshabilitarPrecioCompra"
                            suffix="%"
                            :rules="reglaEditarCompra"
                            v-model="editedItem.LastPurchasePrice"
                            prefix="$"
                            label="Precio de última Compra"
                          ></v-text-field>
                        </v-col>
                        <v-col cols="12" sm="6" md="2">
                          <v-btn
                            :class="classBotonCompra"
                            @click="clickPrecioCompra()"
                          >
                        ...
                        <v-col cols="12" sm="6" md="10">
                          <v-select
                            :disabled="deshabilitarProveedor"
                            :rules="reglaEditarProveedor"
                            v-model="editedItem.Dealer"
                            :items="dealersList"
                            item-text="Email"
                            item-value="_id"
                        ...
                        <v-col cols="12" sm="6" md="2">
                          <v-btn
                            :class="classBotonProveedor"
                            @click="clickProveedor()"
                          >
            ...
            <v-dialog v-model="dialogDelete" max-width="500px">
              <v-card>
                <v-card-title class="headline"
                  >Estas seguro de que quiere eliminar el/los
                  elemento/s?</v-card-title
                >
                <v-card-actions>
                  <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    async getProveedores() {
      await axios.get(urlAPI + "dealer").then(res => {
    ...
    async getRepuestos() {
      await axios.get(urlAPI + "product").then(res => {
    ...
    async getrepuestosStock() {
      await axios.get(urlAPI + "productStock").then(res => {
    ...
    aplicarFiltros() {
      ...
      let SubCategory =
        (this.filtros.SubCategory != null) & (this.filtros.SubCategory != "");
      let Category =
        (this.filtros.Category != null) & (this.filtros.Category != "");
      ...
      for (var i = 0; i < this.repuestos.length; i++) {
        BrandMatches = Brand
          ? this.repuestos[i].Brand === this.filtros.Brand
          : BrandMatches;
        SubCategoryMatches = SubCategory
          ? this.repuestos[i].SubCategory === this.filtros.SubCategory
          : SubCategoryMatches;
        CategoryMatches = Category
          ? this.repuestos[i].Category === this.filtros.Category
          : CategoryMatches;
        SKUMatches = SKU
          ? this.repuestos[i].SKU === this.filtros.SKU
          : SKUMatches;

        cDesdeMatches = cDesde
          ? parseFloat(this.repuestos[i].LastPurchasePrice) >=
            parseFloat(this.filtros.cDesde)
          : cDesdeMatches;
        vDesdeMatches = vDesde
          ? parseFloat(this.repuestos[i].SalePrice) >=
            parseFloat(this.filtros.vDesde)
          : vDesdeMatches;

        cHastaMatches = cHasta
          ? parseFloat(this.repuestos[i].LastPurchasePrice) <=
            parseFloat(this.filtros.cHasta)
          : cHastaMatches;
        vHastaMatches = vHasta
          ? parseFloat(this.repuestos[i].SalePrice) <=
            parseFloat(this.filtros.vHasta)
          : vHastaMatches;
        StatusMatches = Status
          ? this.repuestos[i].Status === this.filtros.Status
          : StatusMatches;
        DealerMatches = Dealer
          ? this.repuestos[i].Dealer.Email == this.filtros.Dealer
          : DealerMatches;
    ...
    async updateManyproducts() {
      await this.selected.forEach(product => {
        let precioVenta = this.deshabilitarPrecioVenta
          ? product.SalePrice
          : this.editedItem.SalePrice;
        let precioCompra = this.deshabilitarPrecioCompra
          ? product.LastPurchasePrice
          : this.editedItem.LastPurchasePrice;
        ...
        if (precioVenta != product.SalePrice) {
          ...
          precioVenta =
            product.SalePrice + (product.SalePrice * precioVenta) / 100;
        }
        if (precioCompra != product.LastPurchasePrice) {
          ...
          precioCompra =
            product.LastPurchasePrice +
            (product.LastPurchasePrice * precioCompra) / 100;
        }
        let proveedor = this.deshabilitarProveedor
          ? product.Dealer
          : this.editedItem.Dealer;
